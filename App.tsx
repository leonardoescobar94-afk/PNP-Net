...
==========================================================
Generated by Polineuropathy-Assistant PM&R Specialist Platform.
`;
  ...
  const handleGeneratePDF = () => {
    ...
    doc.text("Polineuropathy-Assistant PM&R", 14, 20);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(148, 163, 184); // slate-400
    doc.text(t.subtitle, 14, 28);
    ...
    doc.setFont("helvetica", "normal");
    finalY += 7;
    doc.text(`${t.age}: ${patient.age}`, 14, finalY);
    doc.text(`${t.height}: ${patient.height} cm`, 60, finalY);
    ...
    // --- Results Box (Imitating the UI Card) ---
    ...
    doc.rect(14, finalY, 3, 45, 'F');

    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.setTextColor(100, 116, 139); 
    doc.text(t.finalClass.toUpperCase(), 22, finalY + 10);

    doc.setFontSize(12);
    doc.setTextColor(boxColor[0], boxColor[1], boxColor[2]); 
    const splitTitle = doc.splitTextToSize(result.score2.interpretationBody, pageWidth - 50);
    doc.text(splitTitle, 22, finalY + 20);

    if (isAbnormal) {
         doc.setFontSize(11);
         doc.setTextColor(51, 65, 85);
         doc.text(`${t.severity}: ${result.score4.severityLabel}`, 22, finalY + 38);
    }
    ...
    doc.setTextColor(51, 65, 85);
    doc.setFont("helvetica", "bold");
    doc.text(`${t.score2Title} (${result.score2.total}/8 PTS)`, 14, finalY);

    const score2Rows = result.score2.details.map(d => [
        d.nerve,
        d.value === 'NR' ? 'NR' : d.value,
        d.value === 'NR' ? '-' : `P${(d.percentile * 100).toFixed(1)}`,
        d.points
    ]);

    autoTable(doc, {
        startY: finalY + 3,
        head: [['Nervio', 'Valor', 'Percentil', 'Puntos']],
        body: score2Rows,
        theme: 'striped',
    ...
    doc.setFontSize(11);
    doc.text(`${t.score4Title} (${result.score4.total}/8 PTS)`, 14, finalY);
    ...
    if (aiSummary) {
        ...
        doc.text(t.aiAssistant.toUpperCase(), 20, finalY + 10);
        
        doc.setFont("helvetica", "normal");
        doc.setTextColor(51, 65, 85);
        doc.setFontSize(9);
        const splitSummary = doc.splitTextToSize(aiSummary, pageWidth - 40);
        doc.text(splitSummary, 20, finalY + 20);
    }
    ...
    for(let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Generated by Polineuropathy-Assistant PM&R - Page ${i} of ${pageCount}`, pageWidth / 2, 285, { align: "center" });
    }
        ...
        <div className="lg:col-span-8 space-y-6 print:col-span-12">
          ...
          {result && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="bg-slate-900 text-white p-6 md:p-8 rounded-3xl shadow-2xl relative overflow-hidden print:bg-slate-50 print:text-slate-900 print:shadow-none print:border print:border-slate-200 print:p-6">
                
                {/* Decoration */}
                <div className="absolute top-0 right-0 w-48 h-48 bg-blue-600/10 rounded-full -mr-12 -mt-12 blur-3xl print:hidden pointer-events-none"></div>

                {/* Header Row with Buttons */}
                <div className="relative z-20 flex justify-between items-start mb-4">
                  <h2 className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] print:text-slate-500 pt-1">
                    {t.finalClass}
                  </h2>
                ...
                <div className="relative z-10 space-y-4">
                   ...
                   {/* Score 4 Interpretation (Severity) - Solo visible si hay anomalía (Score 2 >= 2) */}
                      ...
                      <div className="mt-2">
                        <p className="text-lg md:text-xl font-bold text-white print:text-slate-900 uppercase tracking-wide">
                          {t.severity}: <span className={
                            result.score4.total >= 7 ? 'text-red-400 print:text-red-700' : 
                            result.score4.total >= 4 ? 'text-orange-300 print:text-orange-700' : 
                            result.score4.total >= 2 ? 'text-yellow-300 print:text-yellow-700' : 
